...
const dataSrc = async (query, id) => {
  try {
    ...
    const source = await fetch(`${commonPath}/token-autocomplete?q=${query}`);
...
export const searchEngine = (query, record) => {
  ...
  if (
    ...
    ((record.name && record.name.toLowerCase().includes(queryLowerCase)) ||
      ...
      (record.address_hash &&
        record.address_hash.toLowerCase().includes(queryLowerCase)) ||
      (record.tx_hash &&
        record.tx_hash.toLowerCase().includes(queryLowerCase)) ||
      (record.block_hash &&
        record.block_hash.toLowerCase().includes(queryLowerCase)))
  ) {
    ...
    if (record.type === "label") {
      ...
      if (record.inserted_at) {
        searchResult += ` (${DateTime.fromISO(record.inserted_at).toFormat(
          "MMM dd, yyyy hh:mm a"
        )})`;
      }
      searchResult += "</div>";
    ...
    searchResult = searchResult.replace(
      re,
      "<mark class='autoComplete_highlight'>$&</mark>"
    );
    return searchResult;
  }
};
const resultItemElement = async (item, data) => {
  ...
  const $tokenIconContainer = $(item).find(
    `#token-icon-${data.value.address_hash}`
  );
  ...
  return {
    ...
    events: {
      input: {
        focus: () => {
          // @ts-ignore
          if (autoCompleteJS && autoCompleteJS.input.value.length)
...
const autoCompleteJS =
  document.querySelector("#main-search-autocomplete") &&
  new AutoComplete(config("main-search-autocomplete"));
// eslint-disable-next-line
const autoCompleteJSMobile =
  document.querySelector("#main-search-autocomplete-mobile") &&
  new AutoComplete(config("main-search-autocomplete-mobile"));

const selection = (event) => {
  const selectionValue = event.detail.selection.value;

  if (
    selectionValue.type === "contract" ||
    selectionValue.type === "address" ||
    selectionValue.type === "label"
  ) {
...
const openOnFocus = (event, type) => {
  ...
  if (query) {
    ...
    getTextAdData().then(({ data: adData, inHouse: _inHouse }) => {
      if (adData) {
        if (type === "desktop") {
          // @ts-ignore
...
const mainSearchAutocompleteObj = document.querySelector(
  "#main-search-autocomplete"
);
const mainSearchAutocompleteMobileObj = document.querySelector(
  "#main-search-autocomplete-mobile"
);

mainSearchAutocompleteObj &&
  mainSearchAutocompleteObj.addEventListener("selection", function (event) {
    selection(event);
  });
mainSearchAutocompleteMobileObj &&
  mainSearchAutocompleteMobileObj.addEventListener(
    "selection",
    function (event) {
      selection(event);
    }
...
mainSearchAutocompleteObj &&
  mainSearchAutocompleteObj.addEventListener("focus", function (event) {
    openOnFocus(event, "desktop");
  });
mainSearchAutocompleteMobileObj &&
  mainSearchAutocompleteMobileObj.addEventListener("focus", function (event) {
